  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const token = envToken || ((setting?.value as { token?: string } | null)?.token ?? "");
        ...
        const sendTelegram = async (chat_id: string, text: string) => {
          ...
          try {
            ...
            return { ok: !!b.ok, error: b.ok ? null : (b.description ?? `HTTP ${r.status}`) };
        ...
        const results = {
          payment_sent: 0, payment_skipped: 0, payment_confirmed: 0,
          lesson_sent: 0, lesson_skipped: 0,
        };
        ...
        for (const p of recentlyPaid) {
          ...
          const { data: exists } = await supabaseAdmin
            ...
            .select("id")
            .eq("student_id", p.student_id).eq("period_month", p.period_month)
            .eq("notification_type", "paid_confirm").limit(1);
          ...
          await supabaseAdmin.from("payment_notifications").insert({
            student_id: p.student_id, period_month: p.period_month, notification_type: "paid_confirm",
            parent_chat_id: student.parent_telegram_chat_id, status: r.ok ? "sent" : "error", error: r.error,
            payload: { text },
          });
        ...
        const debtByStudent = new Map<string, {
          total: number;
        ...
        if (debtByStudent.size) {
          ...
          for (const s of students ?? []) {
            ...
            let bestType: "remind_5" | "remind_3" | "remind_1" | "overdue_0" | "overdue_recurring" | null = null;
            ...
            for (const it of debt.items) {
              ...
              const diffDays = Math.floor((due.getTime() - Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate())) / 86400000);
              let type: "remind_5" | "remind_3" | "remind_1" | "overdue_0" | "overdue_recurring" | null = null;
              ...
              if (type && (!bestType || Math.abs(diffDays) < Math.abs(
                (new Date(bestDue).getTime() - Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate())) / 86400000,
              ))) {
                bestType = type;
                bestItem = it;
            ...
            if (!bestType || !bestItem) { results.payment_skipped++; continue; }

            // Dedup check
            const { data: exists } = await supabaseAdmin
              .from("payment_notifications").select("id")
              .eq("student_id", s.id).eq("period_month", bestItem.period_month)
              .eq("notification_type", bestType)
              .gte("sent_at", `${todayISO}T00:00:00Z`)
              .limit(1);
            if (exists && exists.length) { results.payment_skipped++; continue; }
            ...
            if (!s.parent_notifications_enabled || !s.parent_telegram_chat_id) {
              await supabaseAdmin.from("payment_notifications").insert({
                student_id: s.id, period_month: bestItem.period_month, notification_type: bestType,
                status: "skipped", error: "Chat ID yo'q", payload: { text },
              });
              results.payment_skipped++; continue;
            }
            const r = await sendTelegram(s.parent_telegram_chat_id, text);
            await supabaseAdmin.from("payment_notifications").insert({
              student_id: s.id, period_month: bestItem.period_month, notification_type: bestType,
              parent_chat_id: s.parent_telegram_chat_id,
              due_date: bestDue, status: r.ok ? "sent" : "error", error: r.error, payload: { text },
            });
            if (r.ok) results.payment_sent++; else results.payment_skipped++;
          }
        }
        ...
        const { data: lessons } = await supabaseAdmin
          .from("lessons").select("id, group_id, start_time, end_time, subject_id")
          .eq("is_active", true).eq("day_of_week", dow);
        const groupIds = Array.from(new Set((lessons ?? []).map((l) => l.group_id)));
        if (groupIds.length) {
          const { data: groupStudents } = await supabaseAdmin
            .from("students")
            .select("id, first_name, last_name, group_id, parent_telegram_chat_id, parent_notifications_enabled")
            .in("group_id", groupIds);
          const { data: subs } = await supabaseAdmin.from("subjects").select("id, name");
          const subjName: Record<string, string> = Object.fromEntries((subs ?? []).map((s) => [s.id, s.name]));
          for (const lesson of lessons ?? []) {
            const dedupe = `lesson-${lesson.id}-${todayISO}`;
            const st = (groupStudents ?? []).filter((s) => s.group_id === lesson.group_id);
            for (const s of st) {
              const { data: existing } = await supabaseAdmin.from("parent_notifications")
                .select("id").eq("student_id", s.id).eq("kind", dedupe).limit(1);
              if (existing && existing.length) { results.lesson_skipped++; continue; }
              ...
              if (!s.parent_notifications_enabled || !s.parent_telegram_chat_id) {
                await supabaseAdmin.from("parent_notifications").insert({
                  student_id: s.id, kind: dedupe, channel: "telegram",
                  payload: { text }, status: "skipped", error: "Chat ID yo'q",
                });
                results.lesson_skipped++; continue;
              }
              const r = await sendTelegram(s.parent_telegram_chat_id, text);
              await supabaseAdmin.from("parent_notifications").insert({
                student_id: s.id, kind: dedupe, channel: "telegram",
                payload: { text }, status: r.ok ? "sent" : "error",
                error: r.error, sent_at: r.ok ? new Date().toISOString() : null,
              });
              if (r.ok) results.lesson_sent++; else results.lesson_skipped++;
            }
          }
        }
